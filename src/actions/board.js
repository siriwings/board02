...
/* ARTICLE POST */
export function articlePostRequest(title,contents, publish,count) {
    return (dispatch) => {
        // inform MEMO POST API is starting
        dispatch(articlePost());

        return axios.post('/api/board/', {title,contents, publish,count}, {
            ...
            .then((response) => {
                ...
                count++;
                dispatch(articlePostSuccess(count));
            }).catch((error) => {
...
export function articlePostSuccess(count) {
    return {
 ...
 */
export function articleListRequest(pageNum, articleCount) {
    return (dispatch) => {
            ...
            .then((response) => {
             //   console.log('board response.data : ');
              ...
              //  console.log(response.data.count);
                dispatch(articleListSuccess(response.data.articles,response.data.count,pageNum,articleCount));
            }).catch((error) => {
...
//count: 전체글 개수
export function articleListSuccess(data,count,pageNum,articleCount) {
    return {
